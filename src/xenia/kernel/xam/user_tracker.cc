...
std::optional<TitleInfo> UserTracker::GetUserTitleInfo(
  ...
  if (title_data->last_played.is_valid_last_played_time()) {
...
void UserTracker::UpdateMissingAchievemntsIcons() {
  for (auto& user_xuid : tracked_xuids_) {
    ...
    FlushGpd(user_xuid, spa_data_->title_id());
  }
}
...
std::span<const uint8_t> UserTracker::GetAchievementIcon(
  ...
  return GetIcon(xuid, title_id, entry->image_id);
}
...
bool UserTracker::GetUserSetting(uint64_t xuid, uint32_t setting_id,
  ...
  auto setting = GetSetting(user, setting_id);
  if (!setting) {
    return false;
  }
...
const GpdInfo* UserTracker::GetGpd(UserProfile* profile,
...
std::span<const uint8_t> UserTracker::GetIcon(uint64_t xuid, uint32_t title_id,
  ...
  const auto gpd = GetGpd(user, title_id);
  if (!gpd) {
    return {};
  }

  return gpd->GetImage(image_id);
}
